...
//        progressString = [NSString stringWithFormat:@"%@\n电量",progressString];
     ...
     CGSize expectedSize1 = [elec sizeWithFont:font
                                      minFontSize:5.0
                                   actualFontSize:&actualFontSize
                                         forWidth:maximumSize.width*.6
                                    lineBreakMode:NSLineBreakByWordWrapping];
      font = [font fontWithSize:actualFontSize];
        CGPoint origin1 = CGPointMake(center.x - expectedSize1.width / 2.0, origin.y + expectedSize.height);
      
      [elec drawAtPoint:origin1 withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:_textColor}];
    }
...
-(void)setProString:(NSString *)proString
{
  int pro = [proString intValue];
  
  if (pro<=10) {
    _progressBgColor = [UIColor colorWithRed:(255)/255.0 green:(102)/255.0 blue:(102)/255.0 alpha:(1)];
    [self setTextColor:[UIColor colorWithRed:(255)/255.0 green:(102)/255.0 blue:(102)/255.0 alpha:(1)]];
//    [chargeLable setTextColor:RGB(255, 102, 102, 1)];
  }else if (10 < pro && pro <= 20){
    _progressBgColor  = [UIColor colorWithRed:(255)/255.0 green:(204)/255.0 blue:(51)/255.0 alpha:(1)];
    [self setTextColor:[UIColor colorWithRed:(255)/255.0 green:(204)/255.0 blue:(51)/255.0 alpha:(1)]];
//    [chargeLable setTextColor:RGB(255, 204, 51, 1)];
  }else{
    _progressBgColor = [UIColor colorWithRed:(153)/255.0 green:(204)/255.0 blue:(51)/255.0 alpha:(1)];
    [self setTextColor:[UIColor colorWithRed:(153)/255.0 green:(204)/255.0 blue:(51)/255.0 alpha:(1)]];
//    [chargeLable setTextColor:RGB(153, 204, 51, 1)];
    //        }else{
    //            picView.progressBgColor = RGB(99, 168, 53, 1);
  }
  
  _progress = 1.0 - pro*0.01;
